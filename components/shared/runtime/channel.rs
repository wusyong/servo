    ...
    T: for<'de> Deserialize<'de> + Serialize,
{
    pub fn recv(&self) -> ReceiveResult<T> {
        match *self {
            ...
            Receiver::Mpsc(ref receiver) => receiver.recv().map_err(|_| ReceiveError),
        }
    }
    ...
    pub fn try_recv(&self) -> ReceiveResult<T> {
        match *self {
            ...
            Receiver::Mpsc(ref receiver) => receiver.try_recv().map_err(|_| ReceiveError),
        }
    }
        ...
        T: Send + 'static,
    {
        match self {
            ...
            Receiver::Mpsc(receiver) => receiver.into_inner(),
        }
    }
}
    ...
    T: for<'de> Deserialize<'de> + Serialize,
{
    if opts::multiprocess() {
        ...
        mpmc::channel()
            .map(|(tx, rx)| (Sender::Mpmc(tx), Receiver::Mpsc(rx)))
            .ok()
    }
}
